...
To examine Fitt's Law, I created a web application similar to a Java applet
created by researchers at UC Berkley\cite{applet}\cite{goldberg}. The web app
may be found at \url{http://fitts-law-experiment.herokuapp.com} and the source
code may be found at \url{http://github.com/brianto/fitts-law-experiment}.

The subject is presented a round target placed randomly in one of four possible
locations on the screen. The locations are at the four corners of an imaginary
rectangle. After each successful click of the target, a new target
is shown in one of the other three remaining locations. This series of target
clicking continues for 27 ``good'' clicks.
...
Whenever a participant mis-clicks on a target (clicks on the white space around
the target rather than the target itself) which is considered a ``bad'' click,
the current transition's time is not recorded and a randomly generated
transition not from the queue is presented to the subject. The transition after
the new random transition, if the click is on the target (a ``good'' click),
that click will be registered.
...
data for touch devices is not used in the data analysis. The data for different
pointer types (mouse, trackpad, and tablet) are not differentiated.
...
assumption. Out of 11 total participants, only 4 participants' timing data are
normal.
...
can see that the residuals do not exhibit any pattern. See Figure
\ref{fig:residuals}. Likewise, a Durbin-Watson auto-correlation test against
the data order results in a $p$ value of \Sexpr{values[['independence']]},
which means the residuals have no significant auto correlation. See Figure
\ref{fig:independence}.
...
Shapiro-Wilk normality test. See Figure \ref{fig:normality}. The Shapiro-Wilk
normality test has a $p$ value of \Sexpr{values[['normality']]}, which means
that the data is likely to be normal.
...
\Sexpr{values[['homogeneous']]}, which means that the variances are likely to
be homogeneous (the same).

\section{Results}

\begin{figure}[p]
<<fig=TRUE,echo=FALSE,width=8,height=4>>=
  boxplot(time ~ size, data = fitts)
@
...
\begin{figure}[p]
<<fig=TRUE,echo=FALSE,width=8,height=4>>=
  boxplot(time ~ distance, data = fitts)
@
...
<<results=tex,echo=FALSE>>=
  for (id in unique(fitts$subject)) {
    df <- subset(fitts, subject == id)
    
    print(xtable(df,
      caption = paste("Experiment Data for subject", id),
      label = "fig:data",
      table.placement = "p"))
  }
@
