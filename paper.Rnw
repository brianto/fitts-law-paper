...
normal via the Shapiro-Wilk test for normality\ref{fig:normsubject}.
...
effect of size, $\beta$ represents the effect of distance, and $\epsilon$
...
$Y_{ijk} - \bar Y$ against the order of the data\ref{fig:residuals}. The
...
have significant auto correlation\ref{fig:independence}.
...
Shapiro-Wilk normality test\ref{fig:normality}. The Shapiro-Wilk normality
...
despite its inclusion in Fitt's Law\ref{fig:anova}
