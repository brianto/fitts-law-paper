...
created by researchers at UC Berkley\cite{applet}. The web app may be found at
...
the target rather than the target itsef) which is considered a ``bad'' click,
...
Likewise, a Durbin-Watson auto-correlation test against the data order results in
a $p$ value of 0.3103, which means the residuals have no significant
auto correlation. See Figure \ref{fig:independence}.
...
From Figure \ref{fig:bysize}, we can see that there is some difference between each
size's mean. However in Figure \ref{fig:bydistance}, we can see that the total times
by distance are fairly close in means and there is much overlap between each distance's
IQR.
